 ...
 * @license MIT
 */
import { a as t, c as createConfigLoader, d as resolveRSCEntryFiles, f as ssrExternals, g as preloadVite, h as getVite, i as parse$1, m as defineOptimizeDepsCompilerOptions, n as watch, o as traverse, p as defineCompilerOptions, r as generate, s as configRouteToBranchRoute, u as resolveEntryFiles } from "./typegen-D9G1j4ne.js";
import { u as invariant } from "./routes-GTRillZz.js";
...
function getServerEnvironmentEntries(ctx, record) {
	return Object.entries(record).filter(([name]) => ctx.buildManifest?.serverBundles ? isSsrBundleEnvironmentName(name) : name === "ssr");
}
...
async function getEnvironmentOptionsResolvers(ctx, viteCommand) {
	...
	function getBaseServerOptions({ viteUserConfig }) {
		...
		return mergeEnvironmentOptions(getBaseOptions({ viteUserConfig }), {
			...
			build: {
				...
				copyPublicDir: false,
				rollupOptions: {
					input: viteUserConfig.environments?.ssr?.build?.rollupOptions?.input ?? virtual$1.serverBuild.id,
					output: {
	...
	let environmentOptionsResolvers = { client: ({ viteUserConfig }) => mergeEnvironmentOptions(getBaseOptions({ viteUserConfig }), { build: {
		rollupOptions: {
			...
			output: viteUserConfig?.environments?.client?.build?.rollupOptions?.output ?? { entryFileNames: ({ moduleIds }) => {
				...
				return {
					...
					optimizeDeps: {
						...
						...defineOptimizeDepsCompilerOptions({
							rolldown: { transform: { jsx: "react-jsx" } },
